...
(async () => {
  ...
  // ---------- 场景 N:max_tokens 拉满(小额度被移除,交由上游用模型最大输出) ----------
  await runScenario('max_tokens 拉满', {}, { MAX_TOKENS_FLOOR: '65536' }, async (base, proxy) => {
    let r = await fetch(base + '/v1/responses', {
      method: 'POST', headers: { authorization: 'Bearer k1', 'content-type': 'application/json' },
      body: JSON.stringify({ model: 'deepseek-v4-flash-free', input: 'hi', max_tokens: 200 }),
    });
    let j = await r.json();
    ok('responses 小 max_tokens 被移除(拉满)', j._echo.max_tokens === null, JSON.stringify(j._echo));
    ...
    j = await r.json();
    ok('chat 的 max_completion_tokens 同样被移除', j._echo.max_tokens === null, JSON.stringify(j._echo));

    r = await fetch(base + '/v1/chat/completions', {
      method: 'POST', headers: { authorization: 'Bearer k1', 'content-type': 'application/json' },
      body: JSON.stringify({ model: 'gpt-5.1', messages: [], max_tokens: 80000 }),
    });
    j = await r.json();
    ok('足够大的 max_tokens 不被改动', j._echo.max_tokens === 80000, JSON.stringify(j._echo));
